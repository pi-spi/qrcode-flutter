...
@immutable
class QrImageOptionsEye {
  /// Create a new set of styling options for QR Eye.
  const QrImageOptionsEye({this.shape, this.color});
...
@immutable
class QrImageOptionsData {
  /// Create a new set of styling options for data modules.
  const QrImageOptionsData({
    this.shape,
    this.color,
  });

  /// Eye shape.
  final QrDataShape? shape;
  ...
  @override
  bool operator ==(Object other) {
    if (other is QrImageOptionsData) {
      return shape == other.shape && color == other.color;
    }
    return false;
  }
